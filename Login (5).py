...
class UserAccountManager:
    def __init__(self, db_name='user_accounts.db'):
        ...
        self.createSavedJobsTable()
        self.createDeletedJobApplicationsTable()
    ...
    def createUsersTable(self):
        ...
        self.conn.commit()
    def createSavedJobsTable(self):
      self.cursor.execute('''CREATE TABLE IF NOT EXISTS saved_jobs (
                             job_id INTEGER,
                             student_username TEXT,
                             FOREIGN KEY (job_id) REFERENCES jobs(job_id),
                             FOREIGN KEY (student_username) REFERENCES users(username),
                             PRIMARY KEY (job_id, student_username)
                             )''')
      self.conn.commit()
    def createDeletedJobApplicationsTable(self):
      self.cursor.execute('''CREATE TABLE IF NOT EXISTS deleted_job_applications (
                             job_title TEXT,
                             student_username TEXT,
                             FOREIGN KEY (student_username) REFERENCES users(username),
                             PRIMARY KEY (job_title, student_username)
                             )''')
      self.conn.commit()
    ...
    def memberOptions(self):
      ...
      if choice == "1":
          ...
          print("\nYou have selected to view saved jobs.")
          self.showSavedJobs()
      elif choice == "6":
    ...
    def saveJob(self, job_id):
      if self.logged_in_username is None:
        print("You must be logged in to save a job.")
        return

      try:
        self.cursor.execute("INSERT INTO saved_jobs (job_id, student_username) VALUES (?, ?)", (job_id, self.logged_in_username))
        self.conn.commit()
        print("Job saved successfully.")
      except sqlite3.IntegrityError:
        print("This job is already saved or does not exist.")
    def unsaveJob(self, job_id):
      if self.logged_in_username is None:
        print("You must be logged in to unsave a job.")
        return

      self.cursor.execute("DELETE FROM saved_jobs WHERE job_id = ? AND student_username = ?", (job_id, self.logged_in_username))
      self.conn.commit()
      if self.cursor.rowcount < 1:
        print("This job was not saved or does not exist.")
      else:
        print("Job unsaved successfully.")
    def showSavedJobs(self):
      if self.logged_in_username is None:
        print("You must be logged in to view your saved jobs.")
        return

      self.cursor.execute("SELECT jobs.job_id, title, description, employer, location, salary FROM jobs INNER JOIN saved_jobs ON jobs.job_id = saved_jobs.job_id WHERE saved_jobs.student_username = ?", (self.logged_in_username,))
      saved_jobs = self.cursor.fetchall()
      if saved_jobs:
        print("\nSaved Jobs:")
        for job in saved_jobs:
          print(f"ID: {job[0]}, Title: {job[1]}, Description: {job[2]}, Employer: {job[3]}, Location: {job[4]}, Salary: {job[5]}")
      else:
        print("You have no saved jobs.")
    def deleteJob(self):
        if self.logged_in_username is None:
            print("You need to be logged in to delete a job.")
            return
    
        self.cursor.execute("SELECT job_id, title FROM jobs WHERE creator_username = ?", (self.logged_in_username,))
        ...
        print("\nYour posted jobs:")
        for job in jobs:
            print(f"ID: {job[0]}, Title: {job[1]}")
    
        job_id = input("Enter the ID of the job you wish to delete: ")
        try:
            job_id = int(job_id)
        except ValueError:
        ...
        self.cursor.execute("SELECT * FROM jobs WHERE job_id = ? AND creator_username = ?", (job_id, self.logged_in_username))
        job = self.cursor.fetchone()
        if job is None:
            print("You can only delete jobs that you have posted.")
            return
    
        # Move applied students to deleted_job_applications table before deleting the job
        self.cursor.execute("SELECT applicant_username FROM job_applications WHERE job_title = ?", (job[1],))
        applicants = self.cursor.fetchall()
        for applicant in applicants:
            self.cursor.execute("INSERT INTO deleted_job_applications (job_title, student_username) VALUES (?, ?)", (job[1], applicant[0]))
        
        # Delete the job
        self.cursor.execute("DELETE FROM jobs WHERE job_id = ?", (job_id,))
        self.cursor.execute("DELETE FROM job_applications WHERE job_title = ?", (job[1],))
        self.conn.commit()
        print(f"Job '{job[1]}' and all its applications have been deleted.")
    def notifyDeletedJobs(self):
      if self.logged_in_username is None:
          return 
  
      self.cursor.execute("SELECT job_title FROM deleted_job_applications WHERE student_username = ?", (self.logged_in_username,))
      deleted_jobs = self.cursor.fetchall()
      if deleted_jobs:
          print("\nNotification: The following jobs you applied for have been deleted:")
          for job in deleted_jobs:
              print(f"- {job[0]}")
          # Clear notifications after displaying them
          self.cursor.execute("DELETE FROM deleted_job_applications WHERE student_username = ?", (self.logged_in_username,))
          self.conn.commit()
    ...
    def viewJobDetails(self, jobs):
      ...
      if selection == "1":
          ...
          for job in jobs:
              if job[1] == selection:
                  job_found = True
                  job_id, job_title, job_employer = job[0], job[1], job[3]
                  ...
                  self.cursor.execute("SELECT * FROM saved_jobs WHERE job_id = ? AND student_username = ?", (job_id, self.logged_in_username))
                  saved = self.cursor.fetchone() is not None
                  
                  if saved:
                      print("This job is saved.")
                      save_option = "Unsave this job"
                      save_action = 'U'
                  else:
                      print("This job is not saved.")
                      save_option = "Save this job"
                      save_action = 'S'
                  
                  if not applied:
                      
                    if job_employer == self.logged_in_username:
                        print("You cannot apply for this job since you posted it.")
                    else:
                      action = input(f"Do you want to apply for this job (A), {save_option} ({save_action}), or go back (B)? ").upper()
                      if action == 'A':
                          ...
                          if save_action == 'S':
                              ...
                              self.unsaveJob(job_id)
                  else:
                      if saved:
                          if input("Do you want to unsave this job? (Y/N): ").upper() == 'Y':
                              self.unsaveJob(job_id)
